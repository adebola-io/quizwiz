 ...
 */
function stringifytoBase64(file, extension) {
   return `data:${MIME_TYPES[extension]};base64,${fs.readFileSync(file, {
      encoding: "base64",
   })}`;
}
...
function convertImagetoBase64(image) {
   const assetsFolder = "frontend/src/assets";
   const filePath = `${assetsFolder}/icons/${image}`;
   const extension = extname(image).slice(1);
   return stringifytoBase64(filePath, extension);
}

const image = convertImagetoBase64("wrong.png");
